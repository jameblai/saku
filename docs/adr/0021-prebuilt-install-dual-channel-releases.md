...
- **Install:** user-level binary at `~/.local/bin/saku`; `install.sh` default channel **stable**; `--channel nightly` install-time only. Interactive flow: binary → write `release_channel` in config → **Setup** → user systemd unit → `loginctl enable-linger` → `systemctl --user enable --now`. Non-interactive: binary only + next-step hints.
...
- **systemd:** `~/.config/systemd/user/saku.service`, `Restart=on-failure`.
