...
- File tools may always read/write exactly `~/.saku/MEMORY.md` (and enforce the 2200 cap on write) even if outside Workspace.

## Consequences

- Path helper is security-critical and must be tested (traversal, symlink escape, absolute paths outside root, Memory allowlist exception).
